# lite-samurai: a fast, async prompt based on the Mitsuhiko prompt style.
...
function _lite_samurai_call_prompt_part() {
  if (( $+functions[$1] )); then
    "$1"
  fi
}
...
function _mitsuhiko_precmd() {
  ...
  function async_prompt() {
    ...
    _lite_samurai_call_prompt_part precmd_update_git_vars

    {
      echo -n $'\n'$_MITSUHIKO_PROMPT$' '
      _lite_samurai_call_prompt_part git_super_status
      _lite_samurai_call_prompt_part hg_prompt_info
      _lite_samurai_call_prompt_part virtualenv_prompt_info
      _lite_samurai_call_prompt_part multirust_prompt_info
      _lite_samurai_call_prompt_part transportstatus_prompt_info
      if [[ x$_mitsuhiko_rv != x0 ]]; then
        echo -n " exited %{$fg[red]%}$_mitsuhiko_rv%{$reset_color%}"
      fi
      echo -n $' \n$ '
