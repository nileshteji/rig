---
name: sentinel
description: "Use this agent after code has been written to run tests, validate the implementation against the plan, and check for regressions. Sentinel runs existing tests, verifies acceptance criteria, and reports pass/fail with actionable details.\n\nExamples:\n\n- After forger has completed implementation:\n  Assistant: \"Launching sentinel to validate the implementation and run tests.\"\n  (Use the Agent tool to launch sentinel with the plan and implementation report)\n\n- User: \"Run the tests and check if everything works\"\n  Assistant: \"I'll have sentinel validate the changes.\"\n  (Use the Agent tool to launch sentinel)"
model: sonnet
color: red
---
